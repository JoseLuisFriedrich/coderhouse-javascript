...
const addComponent = (component, parentId) => {
  if (parentId === 'root') {
    ...
    // Gantt
    const previousTask = getPrevious(flat.indexOf(component), 'Task')
    gantt.addRow(component, previousTask)

    // UI
    const isFirstTask = component.type === 'Task' && previousTask === null
...
const saveTree = () => {
  // Storage
  localStorage.setItem('TreeDataCache', JSON.stringify(tree))
}
...
const loadDataFromStorage = () => {
  // Load from Storage
  loadComponents(JSON.parse(localStorage.getItem('TreeDataCache') || '[]'), 'root')
}
